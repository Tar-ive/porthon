...
async def create_actions(
    ...
    try:
        ...
        result = await asyncio.wait_for(generate_actions(chosen, extracted), timeout=30.0)
    except ApiException:
        raise
    except asyncio.TimeoutError:
        raise ApiException(status_code=504, code="timeout", message="Action planning timed out.")
    ...
    return ListObject(data=resources, has_more=False, url="/v1/actions").model_dump(mode="json")
